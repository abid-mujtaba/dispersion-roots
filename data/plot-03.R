...
tikz(file='plot-03.tex', width=7, height=7)
...
p <- subplot(p, "03-a", "0.00")
p <- subplot(p, "03-b", "0.01")
p <- subplot(p, "03-c", "0.10")
p <- subplot(p, "03-d", "0.25")

p <- p + labs(x = "$k_\\perp$", y = "$\\displaystyle \\frac{\\omega}{\\omega_{ce}}$") +
         ggtitle("Dispersion for $\\Lambda_c = 0.01$, $\\kappa_c = 2$, $\\kappa_h = 4$, $\\frac{n_{0h}}{n_{0e}} = 0.5$")
