...
p <- subplot(p, "14-a", "0.00")
p <- subplot(p, "14-b", "0.01")
p <- subplot(p, "14-c", "0.10")
p <- subplot(p, "14-d", "0.25")
p <- p + theme_bw() +
         scale_linetype_manual(name=expression(Lambda[h]), values=c("0.00"="solid", "0.01"="dotted", "0.10"="dashed", "0.25"="dotdash")) +      # The 'name' will be the title of the legend
         ggtitle(expression(paste("Roots of Dispersion Relation for ", Lambda[c], " = 0.02, ", kappa[c], " = 2, ", kappa[h], " = 4, ", frac(n[h0], n[e0]), " = 0.5")))


ggsave(file="plot-14.pdf", plot=p)
