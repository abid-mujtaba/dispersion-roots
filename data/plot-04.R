...
p <- subplot(p, "04-a", "1.6")
p <- subplot(p, "04-b", "2.0")
p <- subplot(p, "04-c", "inf")
p <- p + scale_linetype_manual(name=expression(kappa[h]), values=c("1.6"="solid", "2.0"="dashed", "inf"="dotted")) +      # The 'name' will be the title of the legend
         ggtitle(expression(paste("Roots of Dispersion Relation for ", Lambda, " = 0.2, ", kappa[c], " = 2, ", frac(n[h0], n[e0]), " = 0.5")))
