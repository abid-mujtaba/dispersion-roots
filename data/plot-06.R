...
tikz(file='plot-06.tex', width=7, height=7)
...
p <- subplot(p, "06-a", "1.6")
p <- subplot(p, "06-b", "2.0")
p <- subplot(p, "06-c", "$\\inf$")

p <- p + labs(x = "$k_\\perp$", y = "$\\displaystyle \\frac{\\omega}{\\omega_{ce}}$") +
         ggtitle("Dispersion for $\\Lambda_c = 0.01$, $\\Lambda_h = 0.1$, $\\kappa_h = 4$, $\\displaystyle \\frac{n_{0h}}{n_{0e}} = 0.5$, $\\displaystyle \\frac{T_h}{T_c} = 101.695$")

p <- p + scale_linetype_manual(name="$\\kappa_c$", values=c("1.6"="solid", "2.0"="dashed", "$\\inf$"="dotted"))
