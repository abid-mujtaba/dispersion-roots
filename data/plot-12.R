...
p <- subplot(p, "12-a", "0.00")
p <- subplot(p, "12-b", "0.01")
p <- subplot(p, "12-c", "0.10")
p <- subplot(p, "12-d", "0.25")
p <- p + theme_bw() +
         ...
         ggtitle(expression(paste("Roots of Dispersion Relation for ", Lambda[c], " = 0, ", kappa[c], " = 2, ", kappa[h], " = 4, ", frac(n[h0], n[e0]), " = 0.5")))


ggsave(file="plot-12.pdf", plot=p)
