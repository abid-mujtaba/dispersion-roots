...
p <- subplot(p, "D-3-a", "0.0")
p <- subplot(p, "D-3-b", "0.1")
p <- subplot(p, "D-3-c", "0.2")
p <- p + scale_linetype_manual(name=expression(Lambda), values=c("0.0"="solid", "0.1"="dotted", "0.2"="dashed")) +      # The 'name' will be the title of the legend
         ggtitle(expression(paste("Roots of Dispersion Relation for ", frac(n[h0], n[e0]), " = 0.0, ", kappa[c], " = 2.0 ", k[perp], " = 5.0")))

#p <- p + xlim(0,10)        # Limit x-axis values
p <- p + ylim(-50,50)


ggsave(file="plot-D-3.pdf", plot=p) 
