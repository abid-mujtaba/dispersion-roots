...
p <- subplot(p, "10-a", "10")
p <- subplot(p, "10-b", "100")
p <- subplot(p, "10-c", "1000")
p <- p + scale_linetype_manual(name=expression(frac(T[h], T[e])), values=c("10"="solid", "100"="dashed", "1000"="dotted")) +      # The 'name' will be the title of the legend
         ggtitle(expression(paste("Roots of Dispersion Relation for ", kappa[c], " = 2, ", kappa[h], " = 4, ", Lambda, " = 0.01, ", frac(n[h0], n[e0]), " = 0.0")))

#p <- p + xlim(0,10)        # Limit x-axis values


ggsave(file="plot-10.pdf", plot=p)
