...
sed -i constants.h -e 's/LAMBDA.*$/LAMBDA 0.15/' -e 's/KAPPA_C.*$/KAPPA_C 2.0/' -e 's/N0H_BY_N0E.*$/N0H_BY_N0E 0.5/'


# Now we modify the value of KAPPA_H and generate the corresponding data
sed -i constants.h -e 's/PLOT.*$/PLOT "02-a"/' -e 's/KAPPA_H.*$/KAPPA_H 1.6/'
make data

sed -i constants.h -e 's/PLOT.*$/PLOT "02-b"/' -e 's/KAPPA_H.*$/KAPPA_H 2.0/'
make data
